...
require_once('conexion.php');
	...
	class CrudLinea{
		...
		public function insertar($linea){
			$db=Db::conectar();
			$insert=$db->prepare('INSERT INTO linea values(NULL,:codigo,:descripcion)');
		...
		// método para mostrar todos las lineas
		public function mostrar(){
			$db=Db::conectar();
			$listaLinea=[];
			$select=$db->query('SELECT * FROM linea');
		...
		public function eliminar($id){
			$db=Db::conectar();
			$eliminar=$db->prepare('DELETE FROM linea WHERE ID=:id');
		...
		public function obtenerLinea($id){
			$db=Db::conectar();
			$select=$db->prepare('SELECT * FROM linea WHERE ID=:id');
		...
		public function actualizar($linea){
			$db=Db::conectar();
			$actualizar=$db->prepare('UPDATE linea SET codigo=:codigo, descripcion=:descripcion  WHERE ID=:id');
